					...
					$.each(ques_ids,function(k,ques_id)
					{
						...
						if(typeof v !== 'undefined' && v['desc_id']!='DESC'){
							...
							test_ans[ques_id]=[ques_id,'',0]; //ques_id, ans_option / answer , time_taken
                                                ...
                                                // Parent questions to be added to ques_desc[];
                                                if(v['desc_id']=='DESC'){
                                                    ques_desc[v[0]]=v;
                                                }
					});					
...
function show_question(ques_no)
{
	...
	{		
		...
		ques_box_clone.find('.ques_no').html((ques_no+1)+' / '+ques_length);                
		ques_box_clone.find('.ques').html(ques[ques_no]['ques']);
                ques_box_clone.find('.ques').attr('ques_no',ques[ques_no]['ques_id']);
                // Check parent questions
                if(ques[ques_no]['desc_id']!=''){
                    var q_id=parseInt(ques[ques_no]['desc_id']);
                    ques_box_clone.find('.ques').prepend("<strong>"+ques_desc[q_id]['ques']+" </strong><br /><br />");
                }
