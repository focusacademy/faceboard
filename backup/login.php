    ...
    if (mysql_num_rows($s_res) > 0) 
    {
        ...
        else 
        {
            ...
            if (crypt($pswd, substr($s_r['pswd'], 0, 12)) == $s_r['pswd']) 
            {
				...
				$result = array('status' => 1,'css'=>'alert alert-success','msg'=>'Successfully logged in! <br/> Loading FACEBoard... Please wait !');
            }
        }
     }// USER EXXISTS
 }// SAFE INPUT 
...
<?php
if($result['status']==1)
echo "setTimeout(function(){ location.replace('index.php');},5000);";
?>
</script>
</html>